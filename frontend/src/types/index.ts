 ...
 */
// Record<GroupPlatform, true> 是编译期护栏：GroupPlatform 新增成员而这里漏加会直接
// 报错，避免上游合并后静默漏掉某个平台（kimi/zhipu/deepseek 就是这么漏过一次的）。
const BUILTIN_GROUP_PLATFORMS: Record<GroupPlatform, true> = {
  anthropic: true,
  openai: true,
  gemini: true,
  antigravity: true,
...
export function asGroupPlatform(platform?: string | null): GroupPlatform | undefined {
  // 用 === true 而非 `in`，避免 'toString' 这类原型属性把自由服务名误判成内置平台。
  return platform && BUILTIN_GROUP_PLATFORMS[platform as GroupPlatform] === true
    ? (platform as GroupPlatform)
    : undefined
}

/**
 * 是否为内置账号平台。透传账号的自由服务名（firecrawl 等）返回 false。
 *
 * 判定必须走 BUILTIN_GROUP_PLATFORMS 这一份表——历史上各处手抄平台列表，
 * 上游新增 kimi/zhipu/deepseek 后漏改，导致这些平台被误判成透传账号。
 */
export function isBuiltinAccountPlatform(platform?: string | null): boolean {
  return (
    !!platform &&
    platform !== 'composite' &&
    BUILTIN_GROUP_PLATFORMS[platform as GroupPlatform] === true
  )
}
...
// 内置账号平台 = 内置分组平台去掉 composite（composite 是分组概念，不是账号平台）。
// 从 GroupPlatform 派生而非另抄一份，这样上游新增平台时这里自动跟上。
export type BuiltinAccountPlatform = Exclude<GroupPlatform, 'composite'>

// 透传账号的 platform 是自由服务名（如 firecrawl），/px/:service 直接按它匹配，
// 所以这里必须放开成任意字符串。
//
// 注意：不要写成 `BuiltinAccountPlatform | (string & {})`。那种写法虽然能保留
// 自动补全，但会破坏字面量收窄——`platform === 'kimi'` 之后类型仍带 `string & {}`
// 分支，上游大量 `=== 'kimi' | 'zhipu' | 'deepseek'` 的收窄代码会编译失败。
