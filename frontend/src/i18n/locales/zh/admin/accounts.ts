export default {
    accounts: {
      ...
      upstreamBilling: {
        trustWarning: '此倍率由上游站点针对当前 API Key 自行声明。Wavelet Innovation API Platform 无法验证该值是否与实际扣费一致；上游站点或中间代理可能返回伪造、过期或被篡改的数据。请结合账单、余额变化和实际用量自行核验。',
      ...
      passthrough: {
        title: "透传",
        serviceName: "服务名",
        serviceNameHint: "调用路径为 /px/<服务名>/...，只能用小写字母、数字和连字符，例如 firecrawl",
        baseUrl: "上游 Base URL",
        baseUrlHint: "只填协议和主机（含固定路径前缀），具体路径由下面的路由白名单决定",
        apiKey: "上游 API Key",
        apiKeyHint: "仅用于向上游认证；客户端自己的凭证绝不会被转发",
        routes: "路由白名单（JSON）",
        routesHint: "默认拒绝：只有列在这里的路径才会被转发，其余一律本地 404、不接触上游。path 以 /* 结尾表示前缀匹配；sku 会作为计费单元写入用量记录。",
        routesInvalid: "路由白名单不是合法的 JSON 数组",
        routesRequired: "请至少配置一条路由",
        routeMissingFields: "每条路由都必须有 path 和 sku",
        serviceNameInvalid: "服务名只能包含小写字母、数字和连字符",
      },
      accountType: '账号类型',
      ...
      grok: {
        ...
        ttsTextLabel: 'TTS 文本',
        ttsTextPlaceholder: '例如：Hello from Wavelet Innovation connectivity test.',
