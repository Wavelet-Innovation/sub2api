export default {
    settings: {
      ...
      scheduling: {
        ...
        accountSchedulingThresholdsDescription: '当账号当前原生用量窗口（OpenAI Codex/Anthropic 会话，或 Grok 请求/Token 利用率）达到该百分比时，Wavelet Innovation 会临时将其移出调度，直到窗口重置。填 100 表示禁用。',
      ...
      upstreamBillingProbe: {
        title: '上游倍率自动探测',
        description: '定期获取 API Key 账号所连接上游站点声明的计费倍率；只有另行开启“同步上游声明倍率”的账号才会更新账号倍率。',
        enabled: '启用全局自动探测',
      ...
      site: {
        ...
        siteSubtitlePlaceholder: '安全访问经过批准的 AI 模型与服务',
        apiBaseUrl: 'API 端点地址',
