export default {
    accounts: {
      ...
      upstreamBilling: {
        trustWarning: 'This rate is declared by the upstream site for the current API key. Wavelet Innovation API Platform cannot verify that it matches actual charges. The upstream site or an intermediary may return forged, stale, or modified data. Verify it against bills, balance changes, and actual usage.',
      ...
      passthrough: {
        ...
        serviceNameHint: "Requests go to /px/<service>/... . Lowercase letters, digits and hyphens only, e.g. firecrawl",
        baseUrl: "Upstream base URL",
        baseUrlHint: "Scheme and host only (plus any fixed path prefix); actual paths come from the route allowlist below",
        apiKey: "Upstream API key",
        apiKeyHint: "Used only to authenticate to the upstream; the caller credential is never forwarded",
        routes: "Route allowlist (JSON)",
        routesHint: "Deny by default: only listed paths are forwarded, anything else gets a local 404 without touching the upstream. A path ending in /* is a prefix match; sku becomes the billing unit in usage logs.",
        routesInvalid: "Route allowlist is not a valid JSON array",
        routesRequired: "At least one route is required",
        routeMissingFields: "Every route needs both path and sku",
        serviceNameInvalid: "Service name may only contain lowercase letters, digits and hyphens",
      },
      ...
      grok: {
        ...
        ttsTextPlaceholder: 'Example: Hello from Wavelet Innovation connectivity test.',
        ttsTextDefault: 'Hello from Wavelet Innovation account connectivity test.',
