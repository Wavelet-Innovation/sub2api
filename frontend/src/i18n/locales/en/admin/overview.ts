export default {
    ...
    groups: {
      ...
      openaiLive: {
        ...
        hint: 'When enabled, API keys in this OpenAI group can create and control Live voice sessions. Disabled by default. The Wavelet Innovation API Platform server must run on Apple Silicon macOS with the official ChatGPT app installed; client platforms are unrestricted.',
        unsupportedTitle: 'Current server does not support Live',
        unsupportedMessage: 'This Wavelet Innovation API Platform server cannot generate the required Live attestation. Live will not work even if enabled. Continue anyway?',
